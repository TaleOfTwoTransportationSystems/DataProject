...
With some cleaning, we turn this raw data (which comes to us in the semi-stuctured JSON format) into something we can more easily work with; simple tables.
...
A simple scatter plot of train departures from Porter Square heading inbound over time, to demonstrate the level of detail in this data.
...
Here's one using headways, color-coding lateness (more on this later!); blue is "cool", with low lateness, pink is "warm", with high lateness values:
...
It's interesting to compare this to a version using the raw difference from the benchmark, positive or negate. 
