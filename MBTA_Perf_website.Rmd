...
<div><div>HOLDING SPACE FOR THE VIDEO</div></div>
<iframe width="420" height="315" src="http://www.youtube.com/embed/MxEA5UFpTGw?autoplay=0"></iframe>
'))
```
...
library(shiny)
```
```{r Hardcoded_Stuff, warning=FALSE, echo=FALSE}
...
```
```{r Pull_Performance_Data, warning=FALSE, echo=FALSE}
# In order to associate human-readable names, geographical data, and such
# with MBTA stops, we pull down sets of data regarding the routes.
...
```
```{r redline_split, warning=FALSE, echo=FALSE}
# The Red line is somewhat special in how it appears in the data; it is one line,
# but it has a fork at JFK/UMass which (if you're southbound) goes to Ashmont and
# Braintree. Since the Red line is the one we want to experiment on first, in order
# to get a full list of arrival/departure pairs with which to query the API we do
...
# This gives us a complete set of paired Red line stops. We don't remove the
# Ashmont/Braintree/Main lists because we may want them later.

```
```{r load_existing_data, warning=FALSE, echo=FALSE}
...
```
```{r Pull_Archive_Data, cache=TRUE, warning=FALSE, echo=FALSE}
# When we first started on this project, we believed that the MBTA archives contained
# the travel-time and headway information that we wanted. It turns out we were wrong,
# but this archive data is still useful for finding stop information. This is the code
# to pull it.
# This chunk is slow.  We should probably pull out the few things that are being used
# into a new chunk, then turn off evaluation of the rest.
...
```
```{r stop_lookup, warning=FALSE, echo=FALSE}

# As a final table-modification step, create a simple ordered list of stops with relevant properties, both as a lookup and so stops can
# appear in order in visualizations. It will still produce odd behavior around the Red Line's fork, but more advanced visualizations will
# take this into account. We join this table to our headway and travel time data.
...
```{r, Fetch_Traveltimes, warning=FALSE, eval=FALSE}
# I'm getting 400 errors from these; not sure why. The URL looks fine. If we can fix it, turn eval back on and echo off. Same with the next one.
i <- 0
fromStop <- as.character(70069)
fromStop <- paste("&from_stop=", fromStop, sep="")
toStop <- as.character(70071)
toStop <- paste("&to_stop=", toStop, sep="")
...
foo_travel <- fromJSON(TRequest)[[1]]
foo_travel
```
And here are some headways:
```{r Fetch_Headways, warning=FALSE, eval=FALSE}
fromStop <- as.character(70069)
fromStop <- paste("&stop=", fromStop, sep="")
toStop <- as.character(70071)
toStop <- paste("&to_stop=", toStop, sep="")
...
foo_headways <- fromJSON(TRequest)[[1]]
foo_headways
```

With some cleaning, we turn this raw data (which comes to us in the semi-stuctured JSON format) into something we can more easily work with; simple tables. Here are some examples of what the tables end up looking like and the sorts of columns we have to work with. Most are from the source, a few, like service_dt, we have inferred to make other processing easier.

```{r data_frame_heads, warning=FALSE, echo=FALSE}
# lets try to turn these into a prettier format
kable(head(travel_times %>% mutate(service_d = dt) %>% select(-dep_d, -dep_t, -arr_d, -arr_t, -dt)))
kable(head(headway_times %>% mutate(service_d = dt) %>% select(-prev_route_id, -current_dep_d, -current_dep_t, -previous_dep_d, -previous_dep_t, -dt)))
```
...
```{r vis_train_departures, warning=FALSE, echo=FALSE}
...
Here's one using headways, so that we can color-code by how far from it's benchmark time the train was when it departed the station. Positive values indicate a train was running late, and are encoded tending toward pink. Negative values indicate the train was actually running a bit ahead, and are coded in blue.

```{r vis_headway_timedelta, warning=FALSE, echo=FALSE}
plot(as.numeric(travel_times[travel_times$from_stop == "70065",]$current_dep_dt - travel_times[travel_times$from_stop == "70065",]$dt,unit = 'mins'),
     travel_times[travel_times$from_stop == "70065",]$dt,
     main="Departures from Porter Square heading inbound\n color-coded by travel lateness",
     xlab="Minutes since midnight", ylab = "Date of Trip", pch=".", col = cm.colors(travel_times[travel_times$from_stop == "70065",]$time_delta))
```

```{r vis_headway_timedelta2, warning=FALSE, echo=FALSE}
plot(as.numeric(headway_times[headway_times$from_stop == "70065",]$current_dep_dt - headway_times[headway_times$from_stop == "70065",]$dt,unit = 'mins'),
     ...
     xlab="Minutes since midnight", ylab = "Date of Trip", pch=".", col = cm.colors(headway_times[headway_times$from_stop == "70065",]$time_delta))
```

It's interesting to compare this to a version . 

```{r vis_headway_lateness, warning=FALSE, echo=FALSE}
plot(as.numeric(headway_times[headway_times$from_stop == "70065",]$current_dep_dt - headway_times[headway_times$from_stop == "70065",]$dt,unit = 'mins'),
     headway_times[headway_times$from_stop == "70065",]$dt,
...
Below we plot the delay in seconds, time difference between benchmark time between stops and actual time per individual trip, averaged over all hours:mins from January to April. Rush hour is highlighted in red and green for 7-9am and 5-7pm respectively. In the website, users can select individual stations and explore the delays between each.
```{r Pull_Alerts, echo=FALSE, warning=FALSE}
...
```
```{r Compare_Alerts_To_Performance, echo=FALSE, warning=FALSE}
...
all_lines <- all_lines %>% 
  ...
  select(-extra)
```
``` {r benchmark_vs_actual, echo=FALSE, warning=FALSE}
...
```{r vis_headway_weekday_southbound, warning=FALSE, echo=FALSE}
...
```{r vis_time_delta_boxplot, warning=FALSE, echo=FALSE} 
...
```{r vis_time_delta_density, warning=FALSE, echo=FALSE}
headway_plots <- list()
...
```{r trav_dens, warning=FALSE, echo=FALSE}
...
```{r lateness_dens, warning=FALSE, echo=FALSE}
...
```{r type_of_day, warning=FALSE, echo=FALSE}
...
```{r time_buckets, warning=FALSE, echo=FALSE}
# First, we build the buckets. In this case, we're using hourly intervals, defined by the
...
# Now, we find how many "delay seconds" appear in each of our buckets. The really tricky part
# is accounting for long delays which are partially in one bucket, and partially in another.
...
As you can see the tweets follow a regular format, so with some simple text mining we can extract additional information:
A) Directionality. If the alert relates to routes running southbound, northbound or both. 
B) Severity. Is the alert indicating a minor, moderate or major delay?
C) Location. Which specific T station is the alert related to. 

Here's a sample of what the data looks like once processed:
```{r, echo=FALSE, warning=FALSE}
head(red_line_alert)
```
Here are some summary statistics on the alerts:
```{r}
### Add in a summary table and at least one visualization; maybe a stacked bar chart with one bar per station and one series per severity.
```

Using all of the data we've pulled, we can draw maps of the subway lines Leaflet allowed for easy customization, which we used for overlaying alerts data with the map of the subway lines.  We've added markers to show how many tweets over our time period correspond to the various stations. 
You'll need to zoom in to see this broken out for each individual station. You can also click on individual dots on each line to get the name of the station.
...
orange_line_alert <- orange_line_alert %>% 
  ...
  rename(alerts_at_station_code = after_at_name_code)
```
```{r plot_alerts, echo=FALSE, warning=FALSE}
#Tweets
...
Most alerts are clustered around Park Street, Downtown Crossing, and Backbay. Note from our heatmap above that lateness also seemed worst around Park Street and Downtown Crossing (Back Bay isn't on the Red Line, so it wasn't in that plot.)


### Final Analysis
We feel that, after spending so long sourcing these rich data sets, we've only just scratched the surface when it comes to what we could do with them. We've demonstrated the sorts of measurements one can do, and how even relatively simple visualizations can give insights into the workings of the MBTA's subway lines.

At least two avenues present themselves for further study. First, we expect that a thorough review of the permutations of variance from benchmarks (by station, time of day, etc) could derive insights to help the T operators make both short-term and long-term decisions that would positively impact service quality. The MBTA is certainly already reviewing the efficiency and performance of their system with an eye toward improvements as well as managing the system in real-time to deliver the best service, but access to this level of detail about the functioning of each route and station allows a level of modeling that could enhance both of those endeavors.

Which segues into the second area for further research, which would be apply fancy time-series predictive modeling in real-time to help both T operators and riders. One example, which we had hoped to be able to pursue in this project (but were unable to do so), would be attempting to determine minutes or hours in advance when the system is likely to have problem and take corrective action, or at least alert passengers in time for them to seek alternate routes. One could imagine a situation where, for example, heavy traffic at one end of the Green Line reliably presages a slowdown on the Red Line some time in the future as the delays percolate through the whole system. We of course have no idea if that's actually true, but it's the sort of behavior this level of detail would allow one to attempt to model.
