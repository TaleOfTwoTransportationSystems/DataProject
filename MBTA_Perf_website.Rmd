...
```{r, Fetch_Traveltimes, warning=FALSE, echo=FALSE}
i <- 0
...
kable(head(foo_travel))
```

And here are some headways:
```{r Fetch_Headways, warning=FALSE, echo=FALSE}
fromStop <- as.character(70069)
...
ggplot(headway_times %>% filter(is_weekend == FALSE, direction == "1"), aes(x = as.numeric(dep_time, units="hours"), y = time_delta)) +
...
``` {r benchmark_vs_actual2, echo=FALSE, warning=FALSE}
...
boxplot(time_delta~stop_seq,
     ...
     main="Times between trains by station, weekday Nouthbound",
     xlab="Stop Sequence", ylab = "wait time (seconds)", pch=".", ylim=c(-800, 1600))

boxplot(time_delta~stop_seq,
        data = headway_times %>% filter(is_weekend == TRUE, direction == "0"),
        main="Times between trains by station, Weekend Southbound",
        xlab="Stop Sequence", ylab = "wait time (seconds)", pch=".", ylim=c(-800, 1600))
```
...
ggplot((travel_times %>% filter(direction == 1, is_weekend == FALSE) %>% arrange(stop_seq)), aes(x=travel_time_sec)) +
  geom_density(aes(group = parent_station_name, colour= parent_station_name, fill= parent_station_name), alpha=0.2) +
