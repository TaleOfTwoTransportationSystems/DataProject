...
plot(as.numeric(headway_times[headway_times$from_stop == "70065",]$current_dep_dt - headway_times[headway_times$from_stop == "70065",]$dt,unit = 'mins'),
     ...
     main="Departures from Porter Square heading inbound,\n color-coded by headway performance",
     xlab="Minutes since midnight", ylab = "Date of Trip", pch=".", col = cm.colors(headway_times[headway_times$from_stop == "70065",]$time_delta))
```
With this simple addition we can start to see patterns in the data. Headway lateness seems to cluster; once it happens in a given day it appears to take a while to sort itself out.
...
```{r Pull_Alerts, echo=FALSE, warning=FALSE, message=FALSE}
...
```
```{r Compare_Alerts_To_Performance, echo=FALSE, warning=FALSE, message=FALSE}
...
orange_line_alert <- orange_line_alert %>% 
  mutate(severity = ifelse(grepl("minor",text, ignore.case = TRUE), 1, 
                           ifelse(grepl("moderate",text, ignore.case = TRUE), 2, 
                                  ifelse(grepl("severe",text, ignore.case = TRUE), 3, 0))))
```
```{r Visuals, echo=FALSE, warning=FALSE, message=FALSE}
#Get data
