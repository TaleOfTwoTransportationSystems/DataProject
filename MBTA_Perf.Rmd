...
```{r vis_headway_lateness, eval=FALSE}
...
```{r vis_time_delta_boxplot, eval=FALSE} 
...
#>>>>>>> 269ba259f0fd3dfc0650cbb155566db34954c164
...
leaflet(data = travel_times_alerts3) %>% 
    addTiles() %>% 
    addProviderTiles("CartoDB.Positron") %>%
    addPolylines(data=redAshmontline,~stop_lon,~stop_lat,color="red") %>%
    addPolylines(data=redBraintreeline,~stop_lon,~stop_lat,color="red") %>%
...
# stop doing CSV reads unless there's also code here to create the underlying data set
