...
Measuring ridership on the T requires turnstile data, which was present in our first data source but turned out to not be granular enough to associate directly with alerts, as the question would require.

ASDF ASDF I remember seeing below that we did manage do do some association between alerts and travel_times; can someone summarize how close we got to answering these questions and what we learned about the relationship (if anything)?
...
These three had to be dropped when we determined the hubway data was too old to overlap with our other data sets. Rather than pursuing the Hubway data (which was pretty thoroughly explored in the Hubway Data Challenge: [link]http://hubwaydatachallenge.org/)
