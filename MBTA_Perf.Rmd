...
if(length(ls(pattern="finished_dataset")) > 0) {
...
} else if (file.exists("train_travel_times.csv")) {
    ...
    write.csv(finished_dataset, "train_travel_times.csv") #so others don't need to pull the data again
}
```

asdf

```{r Pull_Archive_Data, cache=TRUE}
# splitting date & time
