...
Before we can work with the travel time or headways data, we need to clean up some data types.
```{r}
...
allstops<-Tarchive_stops %>% 
  ...
  filter(is.na(stop_code)==FALSE) %>%
  mutate(from_stop=stop_code,to_stop=stop_code)
>>>>>>> e4877e06835678bc97a7037203f0838d15baa1c0
...
<<<<<<< HEAD
  left_join(allstops,by="from_stop") %>% select(-to_stop.y,-stop_name.x) %>%
  rename(to_stop=to_stop.x, start_stop=stop_name.y, start_lat=stop_lat, start_long=stop_lon)
=======
...
>>>>>>> ba934a78877a16836195871e8225d1797fb95dc6
...
#Map of Boston
lon_range <- extendrange(all_lines$stop_long)
lat_range <- extendrange(all_lines$stop_lat)
...
## The static map is somewhat boring, so we want to make an interactive one
...
<<<<<<< HEAD
red_line_alert<-read.csv("https://raw.githubusercontent.com/TaleOfTwoTransportationSystems/DataProject/master/red_line_tweets_enhanced.csv")
...
<<<<<<< HEAD
allstops<-allstops %>% rename(stop_code=from_stop) %>% select(-to_stop)
